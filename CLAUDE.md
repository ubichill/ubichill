...
- テストを書くこと。UIコンポーネントのテストは不要でロジックのテストを書くことで少し意地悪なテストをして。それと副作用やdb依存などを分離することでテストしやすくして
- **スタイリング境界**:
...
マージコミットはなるべく減らしてmainブランチを綺麗に保ちたい、PRは人間がレビューすべきものは必ずレビューさせること。さらにラベルでpreviewをつけるといい
