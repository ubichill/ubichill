...
export const PenCanvasSystem: System = (entities: Entity[], _dt: number, events: WorkerEvent[]) => {
    ...
    Ubi.canvas.frame(CANVAS_TARGET, {
        activeStroke:
            isPenHeld && state.isDrawing && state.currentStroke.length > 1
                ? { points: state.currentStroke, color: state.color, size: state.strokeWidth }
                : null,
        cursor: isPenHeld
            ? {
                  x: state.cursorX,
