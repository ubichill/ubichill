...
(() => {
  ...
  var PenCanvasSystem = (entities, _dt, events) => {
    ...
    const drawEntity2 = entities.find((e) => e.hasComponent(DrawState.name));
    if (!drawEntity2) return;
    const state = drawEntity2.getComponent(DrawState.name);
    if (!state) return;
    const isPenHeld = state.heldPenId !== null;
    Ubi.canvas.frame(CANVAS_TARGET, {
      activeStroke: isPenHeld && state.isDrawing && state.currentStroke.length > 1 ? { points: state.currentStroke, color: state.color, size: state.strokeWidth } : null,
      cursor: isPenHeld ? {
        x: state.cursorX,
        y: state.cursorY,
        color: state.color,
        size: Math.max(CURSOR_RADIUS * 2, state.strokeWidth),
        shape: "custom",
        // 座標原点 = カーソル先端、45° 傾き
        rotation: -Math.PI / 4,
        pathFills: [
          { d: "M0,0 L-3,-8 L3,-8 Z", fill: "#888" },
          { d: "M-3,-32 h6 v24 h-6 Z", fill: state.color },
  ...
  var PenInputSystem = (entities, _dt, events) => {
    const drawEntity2 = entities.find((e) => e.hasComponent(DrawState.name));
    if (!drawEntity2) return;
    const state = drawEntity2.getComponent(DrawState.name);
  ...
  var PenWatchSystem = (entities, _dt, events) => {
    const drawEntity2 = entities.find((e) => e.hasComponent(DrawState.name));
    if (!drawEntity2) return;
    for (const event of events) {
      ...
      if (isHeldByMe) {
        ...
        if (data.color !== void 0) state.color = data.color;
        if (data.strokeWidth !== void 0) state.strokeWidth = data.strokeWidth;
  ...
  var drawEntity = Ubi.local.createEntity("draw-state");
