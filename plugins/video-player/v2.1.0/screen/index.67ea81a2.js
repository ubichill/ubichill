  ...
  var ScreenSystem = (_entities, _dt, events) => {
    for (const event of events) {
      ...
      if (event.type === "media:timeUpdate") {
        ...
        const now = Date.now();
        if (screen.local.isPlaying && now - screen.local.lastBroadcastAt >= 2e3) {
      ...
      if (event.type === "media:ended") {
        ...
        const nextIdx = screen.local.shuffle ? Math.floor(Math.random() * screen.local.playlist.length) : screen.local.currentIndex + 1;
