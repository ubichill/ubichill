import type { CursorState, PluginGuestCommand, UserStatus } from '@ubichill/sdk';
import { usePluginWorker, useSocket } from '@ubichill/sdk';
    ...
    const { sendEvent } = usePluginWorker({
        ...
        onCommand: (command: PluginGuestCommand) => {
            if (command.type === 'SCENE_UPDATE_CURSOR') {
                setSmoothPosition({ x: command.payload.x, y: command.payload.y });
            }
        },
    });
