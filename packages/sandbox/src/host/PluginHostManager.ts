 ...
 * Node.js / ブラウザいずれの環境でも動作し、React に依存しない。
 *
...
export class PluginHostManager<TMsg extends PluginWorkerMessage = PluginWorkerMessage> {
    ...
    private _startTickLoop(): void {
        this._handleVisibilityChange = this._handleVisibilityChange_impl.bind(this);
        document.addEventListener('visibilitychange', this._handleVisibilityChange);
        if (!document.hidden) {
            this.animationFrameId = requestAnimationFrame(this._animate.bind(this));
        } else {
            this.intervalId = setInterval(this._intervalTick.bind(this), this.intervalMs);
        }
    }

    private _handleVisibilityChange_impl(): void {
