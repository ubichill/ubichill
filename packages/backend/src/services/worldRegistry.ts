...
async function resolveGitHubRegistryUrls(treeUrl: string, token?: string): Promise<string[]> {
    const match = GITHUB_TREE_RE.exec(treeUrl);
    if (!match) return [];
    ...
    return items
        .filter((item) => item.type === 'file' && /\.(yaml|yml)$/.test(item.name) && item.download_url)
...
class WorldRegistry {
    ...
    constructor() {
        ...
        this.registryToken = process.env[ENV_KEYS.WORLDS_REGISTRY_TOKEN];
    }
    ...
    async reloadWorld(worldId: string): Promise<boolean> {
        ...
        return this._reloadEntry(entry);
    }
    ...
    private async _buildIndexFromScan(): Promise<void> {
        ...
        const prevOrder = new Map(entries.map((e, i) => [e.name, i]));
        entries.sort((a, b) => (prevOrder.get(a.name) ?? 999) - (prevOrder.get(b.name) ?? 999));
    ...
    private async _reloadEntry(entry: WorldIndexEntry): Promise<boolean> {
        const filePath = path.join(this.worldsDir, entry.file);
        if (!fs.existsSync(filePath)) return false;
        try {
            const newEntry = this._parseYamlToEntry(filePath, entry.file);
    ...
    private async _seedWorldFromUrl(url: string): Promise<void> {
        ...
        if (this.registryToken && url.includes('raw.githubusercontent.com')) {
