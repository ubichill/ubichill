 ...
 * - **受信側**: `onMessage(msg: ModWorkerMessage<TMap>)` — `msg.type` で switch すると payload が自動で絞り込まれる
 *
 * @example
 * ```ts
 * type PenPayloads = {
 ...
 * // 受信（判別可能ユニオン）
 * type PenMsg = ModWorkerMessage<PenPayloads>;
 * ```
 */
export type ModWorkerMessage<TPayloadMap extends Record<string, unknown> = Record<string, unknown>> = {
 ...
 * Fire & Forget: mod Worker からホストへログを転送する。
 ...
 * Canvas に描画するカーソルデータ。
 *
 * shape === 'custom' の場合、modが pathFills / pathStrokes に
 ...
 *   再生・一時停止を無効化し、再生制御をmod命令（Ubi.media.play/pause）のみに限定する。
 * enabled=true: デバイス操作と PiP を解放する（modが明示的に許可した場合のみ）。
 */
...
// mod開発者はこれらをリスナー登録して処理します。
// ============================================

/**
 * [Lifecycle] modの初期化完了時に Worker へ送られるイベント。
 * 初期化後は Ubi.worldId / Ubi.myUserId / Ubi.modId / Ubi.entityId で参照可能。
 *
 ...
 * - `componentType`: 自 Worker の Component 型 (`modId:componentName`)。
 *
 * @param worldId              所属ワールドのID
 * @param myUserId             自分のユーザーID
 * @param code                 実行するmodコード (Sandbox内部のみ)
...
export type EvtLifecycleInit = {
    ...
    payload: {
        ...
        /**
         * mod が `watchEntityTypes` で宣言したコンポーネント型の、Worker 起動時点でのスナップショット。
         * SDK がmodコード実行前に state.local へ同期反映するため、
         * 遅れて入室したユーザーでも初期値が揃った状態でmodが立ち上がる。
         *
...
export type EvtRpcResponse = {
    ...
    data?: unknown;
    error?: string;
    /** 失敗理由の machine-readable code。SDK が UbiError.code に載せてmodへ伝える。 */
 ...
 *
 * GenericModHost が WorkerModDefinition.watchEntityTypes と照合し自動送信する。
 ...
 * mod開発者は ECS System の `events` 引数から `EcsEventType.INPUT_*` で取得できる。
 */
