...
| `Ubi.entity.*` | 読み取り・async | 自 Entity (GameObject) 上の他 Component / 親 / 子 へのアクセス |
...
| `ENTITY_GET_SIBLINGS` | RPC | 自 Entity 上の他 Component 取得 (`Ubi.entity.getSiblings`) |
| `ENTITY_GET_PARENT` | RPC | 親 Entity 上の Component 取得 (`Ubi.entity.getParent`) |
| `ENTITY_GET_CHILDREN` | RPC | 子 Entity 上の Component 取得 (`Ubi.entity.getChildren`) |
| `ENTITY_QUERY_SUBTREE` | RPC | 自 Entity + 子孫から type 一致を収集 (`Ubi.entity.queryInSubtree`) |
