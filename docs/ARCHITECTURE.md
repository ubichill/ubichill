...
Browser (Main Thread)
  └─ React Host
       └─ ModHostManager          packages/sandbox
            │  postMessage
            ▼
...
modは ECS System として実装し、毎フレーム `entities` / `deltaTime` / `events` を受け取る。
...
| `EVT_CUSTOM` | mod独自イベント（Host → Worker） |
...
modの UI ロジックは **Web Worker** の中で完結する。
...
```mermaid
sequenceDiagram
    participant W as Worker (Mod)
    participant H as ModHostManager
    participant R as useModUI
