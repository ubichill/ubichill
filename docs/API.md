...
// ──── World (横断・escape hatch) ─────────────────────────
await Ubi.world.get(id) / Ubi.world.query(type) / Ubi.world.update(id, patch)

// ──── トップレベル shortcut ─────────────────────────────
await Ubi.spawn(entity)                            // 新規 Entity (parent 任意指定)
await Ubi.destroy(id)                              // 他 Entity 削除
...
| `scene:update` | `Ubi.spawn` / `Ubi.destroy` / `Ubi.entity.*` / `Ubi.world.update` / `Ubi.state.sync` の書き込み全般 |
