...
ゼロトラストなプラグイン権限。開発者は権限を宣言せず、使用 API から自動導出し、実行時に
...
- fetch は接続先ドメインごとに承認。プラグイン自身のアセット領域は無承認
- 設定画面：シールドレベル（なし / 確認 / 厳格な確認 / 拒否）、プラグイン別の許可の確認・取り消し
- 内部 API 叩きの抜け道封鎖（相対 fetch をアセット領域に限定）

### Phase 3 — capability 自動生成（✅ 完了）

- ビルド時に Worker コードを静的解析して capability を自動生成（`plugin.json` 手書き廃止）
