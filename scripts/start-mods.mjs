...
if (existsSync(modsDir)) {
    ...
    for (const entry of entries) {
        if (entry.isDirectory()) {
            const composeFile = join(modsDir, entry.name, 'docker-compose.yml');
            if (existsSync(composeFile)) {
                composeFiles.push('-f', composeFile);
                console.log(`Found mod definition: ${composeFile}`);
            }
        }
    }
}
...
if (composeFiles.length === 0) {
    console.log('No mod docker-compose configuration files found.');
    process.exit(0);
}
